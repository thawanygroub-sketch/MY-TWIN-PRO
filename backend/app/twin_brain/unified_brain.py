"""Unified Twin Brain v10 — أهمية ديناميكية (7.6)، صمت قراري (4.4)، حدود fail-safe."""
import logging
...
class UnifiedTwinBrain:
    ...
    def _importance(self, intensity, bond):
        ...
        return min(100, imp)
    async def process(self, user_id, message, lang="ar", perception=None,
                      history=None, device_info=None, tier="free", mode=None) -> Dict[str, Any]:
        ...
        strategy = {"goal":intent["goal"],"tone":behavior["tone"],"personality_dna":dna,
                    "emotion":effective,"engine_context":f"[STATE] Emotion: {effective} | Bond: {bond} | Tier: {tier}"}
        ...
        await unified_memory_engine.store(user_id=user_id, content=message, reply=reply,
            emotion=effective, importance=importance, lang=lang)
