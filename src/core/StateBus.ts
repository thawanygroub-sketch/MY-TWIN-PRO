...
const clampLevel = (v: unknown): PresenceLevel => {
  const n = Number(v);
  if (!Number.isFinite(n)) return 2;
  return Math.max(0, Math.min(9, Math.round(n))) as PresenceLevel;
};
export class StateBusClass {
  ...
  update(p: Partial<TwinState>): void { this.apply(p); }
  private apply(p: Partial<TwinState>) {
    this.prevState = { ...this.state };
    this.state = { ...this.state, ...p };
    this.subscribers.forEach(s => { try { s(this.state, this.prevState); } catch(e){ console.warn(e); } });
  }
  select<T>(sel: (s: TwinState) => T): T { return sel(this.state); }
  subscribe(sub: StateSubscriber): () => void { this.subscribers.add(sub); return () => this.subscribers.delete(sub); }
  subscribeTo<T>(sel: (s: TwinState) => T, cb: (v: T) => void): () => void {
    return this.subscribe(s => cb(sel(s)));
  }
  on(event: string, cb: (e: string, d: any) => void): () => void {
    if (!this.eventListeners.has(event)) this.eventListeners.set(event, []);
    this.eventListeners.get(event)!.push(cb);
    return () => { const a = this.eventListeners.get(event); if (a) { const i = a.indexOf(cb); if (i>-1) a.splice(i,1); } };
  }
  ...
  /** يقرأ Response Envelope الموحد (presence_state/twin_emotional_state/twin_state_update)
      مع fallback آمن للحقول المسطحة — لا NaN أبدًا. */
  updateFromUnifiedResponse(response: any): void {
    ...
    this.apply({
      ...
      relationship: { bondLevel: Number(r.bond_level ?? response.bond_level ?? this.state.relationship.bondLevel) || 0,
        ...
        trustScore: Number(r.trust ?? 50)/100 || 0.5,
        firstContactTimestamp: this.state.relationship.firstContactTimestamp },
  ...
  reset(): void { this.prevState = { ...DEFAULT_STATE }; this.state = { ...DEFAULT_STATE }; this.subscribers.forEach(s => s(this.state, this.prevState)); }
}
