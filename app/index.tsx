...
import { View, Image, Text, StyleSheet, Dimensions, Animated, Easing, StatusBar } from 'react-native';
import { router } from 'expo-router';
import { Canvas, Circle, Path, RadialGradient, vec, BlurMask, Paint, useImage } from '@shopify/react-native-skia';
import { useSharedValue, useFrameCallback, useAnimatedStyle } from 'react-native-reanimated';
...
export default function Index() {
  const logoOpacity = useRefAnim(0); const logoScale = useRefAnim(0.92);
  const soulSyncOpacity = useRefAnim(0); const exitO = useRefAnim(1);
  ...
  useFrameCallback((fi) => {
    const t = fi.timeSinceFirstFrame * 0.001; time.value = t;
  ...
  useEffect(() => {
    ...
    const seq = async () => {
      ...
      sparkA.value = AnimatedSeq(sparkA, 0.9, 700);
      play('first_breath');
      await new Promise(r => setTimeout(r, 1200));
      Animated.timing(logoOpacity, { toValue: 1, duration: 1000, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start();
      Animated.spring(logoScale, { toValue: 1, damping: 14, stiffness: 120, useNativeDriver: true }).start();
      play('awakening_glow');
      await new Promise(r => setTimeout(r, 1500));
      Animated.timing(soulSyncOpacity, { toValue: 0.65, duration: 1500, useNativeDriver: true }).start();
      await new Promise(r => setTimeout(r, 1800));
      play('workspace_enter');
      ringO.value = 0.9;
      ringR.value = width * 1.4;
      setTimeout(() => { ringO.value = 0; }, 300);
      Animated.timing(exitO, { toValue: 0, duration: 600, delay: 350, useNativeDriver: true }).start();
    ...
    <View style={st.root}>
      ...
      <Animated.View style={[st.fill, fadeStyle]}>
        <Canvas style={st.fill}>
          ...
          <Circle cx={CX} cy={CY} r={30} opacity={sparkA}>
            <Paint><RadialGradient c={vec(CX, CY)} r={30} colors={['#FFFFFF', '#A855F7', '#00000000']} /></Paint>
          </Circle>
          ...
          <Circle cx={CX} cy={CY} r={height * 0.75}>
            <Paint><RadialGradient c={vec(CX, CY)} r={height * 0.75} colors={['#00000000', '#00000000', '#000005E6']} /></Paint>
          </Circle>
        </Canvas>
        <Animated.View style={[st.logoWrap, { opacity: logoOpacity, transform: [{ scale: logoScale }] }]}>
          ...
          <Animated.Text style={[st.brand, { opacity: soulSyncOpacity }]}>by SOULSYNC</Animated.Text>
        </Animated.View>
      </Animated.View>
    </View>
  );
}
function useRefAnim(v: number) { const r = React.useRef(new Animated.Value(v)).current; return r; }
function AnimatedSeq(sv: Animated.Value, to: number, d: number) {
  Animated.timing(sv, { toValue: to, duration: d, useNativeDriver: true }).start();
  return to;
}
