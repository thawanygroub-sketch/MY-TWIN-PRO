...
import {
  ...
  Platform, TouchableOpacity, ScrollView, Dimensions,
} from 'react-native';
...
const SPACE = { sm: 8, md: 14, lg: 20, xl: 32 };
const RADIUS = { input: 24 };
const LIVING = {
...
export default function LivingWorld() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      setMessages(prev => {
        const u = [...prev];
        u[u.length - 1] = { ...u[u.length - 1], text: response?.reply || LIVING.SERVER };
        return u;
      });
    } catch {
      EventBus.emit('AI_FINISH_THINKING', {});
      setMessages(prev => {
        const u = [...prev];
        u[u.length - 1] = { ...u[u.length - 1], text: LIVING.NETWORK };
        return u;
      });
    }
  ...
  return (
    ...
    >
        ...
        {messages.map(msg => (
          <View
            ...
            style={[
              ...
              msg.sender === 'user' ? styles.userBubble : styles.twinBubble,
              { backgroundColor: msg.sender === 'user' ? colors.accent + '20' : colors.card },
            ]}
          >
      ...
      <View style={[styles.inputContainer, { backgroundColor: colors.card }]}>
        ...
        />
        <TouchableOpacity onPress={handleSend} disabled={isThinking}>
          <Send size={22} stroke={isThinking ? colors.textSecondary : colors.accent} />
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
  );
